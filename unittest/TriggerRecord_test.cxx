...
BOOST_AUTO_TEST_CASE(ExistingHeader)
{
  ...
  BOOST_REQUIRE_EQUAL(record.header_ref().get_error_bit(static_cast<TriggerRecordErrorBits>(0)), false);
  BOOST_REQUIRE_EQUAL(record.header_ref().get_error_bit(static_cast<TriggerRecordErrorBits>(1)), true);
